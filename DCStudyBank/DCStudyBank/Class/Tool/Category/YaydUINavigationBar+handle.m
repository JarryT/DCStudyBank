...
- (void)navBarMyLayerHeight:(CGFloat)height isOpaque:(BOOL)opaque {
    ...
    [self.myNavView setFrame:CGRectMake(0, 0,KScreenWidth, height)];
    ...
    if (self.myNavView) {
        ...
        if (hidden) {
            if (!self.lineClearImage) {
                self.lineClearImage = [UIImage imageWithColor:[UIColor clearColor]];
            }
        } else {
            if (self.lineClearImage) {
...
- (void)clearSystemLayerIsOpaque:(BOOL)opaque{
    ...
    //去掉系统底线，使用自定义底线
    if (!self.lineClearImage) {
        self.lineClearImage = [UIImage imageWithColor:[UIColor whiteColor]];
        [self setShadowImage:self.lineClearImage];
    }
}
@end
...
//        [_bottomLine setBackgroundColor:kTableViewCellSpColor];
        [_bottomLine setBackgroundColor:[UIColor clearColor]];
    }
    return _bottomLine;
}
@end
