...
- (void)layoutSubviews {
    ...
    switch (self.btnType) {
        case TOP:
        {
            ...
            self.titleLabel.center = CGPointMake(self.frame.size.width / 2, CGRectGetMaxY(self.imageView.frame) + titleFrame.size.height / 2 + 10);
        }
            break;
        case BOTTOM:
        {
        ...
        case RIGHT:
        {
            ...
            self.imageView.center = CGPointMake(CGRectGetMaxX(self.titleLabel.frame) + 2 + imageFrame.size.width / 2, self.frame.size.height / 2);
        }
            break;
        default:
            break;
